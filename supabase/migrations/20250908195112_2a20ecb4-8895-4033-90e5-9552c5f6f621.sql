-- Drop all existing award_points functions to clean up duplicates
DROP FUNCTION IF EXISTS public.award_points(uuid, text, text);
DROP FUNCTION IF EXISTS public.award_points(uuid, text, text, integer);
...
-- Create the main 5-parameter function
...
-- Create the 4-parameter wrapper for triggers that call award_points(uuid, text, text, jsonb)
CREATE OR REPLACE FUNCTION public.award_points(
  p_user_id uuid,
  p_activity_type text,
  p_description text,
