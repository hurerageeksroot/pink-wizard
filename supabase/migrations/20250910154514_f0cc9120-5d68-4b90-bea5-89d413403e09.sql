-- Completely redesign the leaderboard update to avoid DELETE operations
...
AS $function$
BEGIN
  -- Use INSERT ... ON CONFLICT DO UPDATE (UPSERT) instead of DELETE + INSERT
  ...
  -- Remove stale entries for users who are no longer active participants
  DELETE FROM public.leaderboard_stats 
  WHERE user_id NOT IN (
    SELECT user_id FROM public.user_challenge_progress WHERE is_active = true
  );
