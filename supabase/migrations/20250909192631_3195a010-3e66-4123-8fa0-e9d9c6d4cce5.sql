-- Fix final variable name conflict in the daily progress update function
...
BEGIN
  ...
  LOOP
    ...
    SELECT COALESCE(ucp.longest_streak, 0) 
    INTO user_max_streak
    FROM public.user_challenge_progress ucp
    WHERE ucp.user_id = user_record.user_id;
    
    -- Update longest streak if current is longer
    user_max_streak := GREATEST(user_current_streak, user_max_streak);
    ...
    SET 
      ...
      longest_streak = user_max_streak,
