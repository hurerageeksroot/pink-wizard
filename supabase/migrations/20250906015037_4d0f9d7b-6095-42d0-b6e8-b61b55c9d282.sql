-- Insert badges with properly cast enum values
...
-- Create guaranteed_rewards_definition table if needed
...
-- Enable RLS and create policies only if table exists and needs them
DO $$ 
BEGIN
  -- Enable RLS if not enabled
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'guaranteed_rewards_definition') 
     AND NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'guaranteed_rewards_definition' AND rowsecurity = true) THEN
