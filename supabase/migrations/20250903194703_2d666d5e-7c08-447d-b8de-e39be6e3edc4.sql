-- Update points leaderboard function to handle ties properly
...
-- Update revenue leaderboard function to handle ties properly
...
-- Update challenge leaderboard stats function to handle ties properly
...
BEGIN
  ...
  -- First, ensure all profiles have a leaderboard entry
  ...
  -- Update/calculate leaderboard stats for ALL users with profiles
  ...
  comprehensive_progress AS (
    SELECT 
      p.id as user_id,
      -- Calculate overall progress across ALL task types including project goals
      COALESCE(
        (
  ...
  -- Update rank positions for ALL users who opted in to leaderboard using DENSE_RANK to handle ties
  ...
  -- Set rank_position to NULL for users who opted out of leaderboard
  UPDATE public.leaderboard_stats ls
  SET rank_position = NULL 
  WHERE NOT EXISTS (
    SELECT 1 FROM public.profiles p 
    WHERE p.id = ls.user_id AND p.show_in_leaderboard = true
  );
