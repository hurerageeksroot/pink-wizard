-- Fix search path security warning for admin_complete_daily_task function
-- Make search_path immutable for security compliance
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''  -- Immutable search path for security
AS $$
DECLARE
  task_result JSONB;
...
BEGIN
  ...
  SELECT public.toggle_user_daily_task(
    target_user_id,
    task_definition_id,
    challenge_day_param,
