...
-- Create a proper INSERT policy with WITH CHECK condition using the specific function signature
CREATE POLICY "Users can create their weekly tasks" 
ON public.user_weekly_tasks 
FOR INSERT 
WITH CHECK ((auth.uid() = user_id) AND public.user_can_write(auth.uid()));

-- Also ensure there's a DELETE policy for completeness  
CREATE POLICY "Users can delete their weekly tasks" 
ON public.user_weekly_tasks 
FOR DELETE 
USING ((auth.uid() = user_id) AND public.user_can_write(auth.uid()));