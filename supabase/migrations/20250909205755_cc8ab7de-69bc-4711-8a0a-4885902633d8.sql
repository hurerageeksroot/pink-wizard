-- Part 1: Fix AI quota robustness and challenge participant access

-- Fix get_my_ai_quota to be more robust and handle missing subscribers table
...
BEGIN
  ...
  SELECT user_is_challenge_participant(v_user_id) INTO v_is_challenge_participant;
  
  -- SHORT CIRCUIT: If admin or challenge participant, return unlimited immediately
  ...
  -- For regular users, safely check subscribers table (guard against missing table)
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Subscribers table doesn't exist or other error, use free tier defaults
    v_subscribed := false;
    v_sub_tier := NULL;
  END;
...
-- Part 2: Fix daily tasks reliability

-- First, clean up duplicate user_daily_tasks entries
WITH duplicate_tasks AS (
  SELECT id, 
    ROW_NUMBER() OVER (PARTITION BY user_id, task_id, challenge_day ORDER BY created_at DESC) as rn
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_daily_tasks_unique
  ON public.user_daily_tasks (user_id, task_id, challenge_day);

-- Part 3: Create improved upsert function for daily tasks
...
GRANT EXECUTE ON FUNCTION public.ensure_user_daily_task_exists(UUID, UUID, INTEGER) TO authenticated;