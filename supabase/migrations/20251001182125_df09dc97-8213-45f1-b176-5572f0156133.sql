-- Add skip_milestone_checks parameter to award_points function
-- This allows core CRM operations to award points without triggering complex milestone logic
CREATE OR REPLACE FUNCTION public.award_points(
  p_user_id uuid, 
  p_activity_type text, 
  p_description text, 
  p_metadata jsonb DEFAULT '{}'::jsonb,
  p_skip_milestone_checks boolean DEFAULT false
)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Only run milestone checks if not skipped
  IF NOT p_skip_milestone_checks THEN
    -- Check for badges
