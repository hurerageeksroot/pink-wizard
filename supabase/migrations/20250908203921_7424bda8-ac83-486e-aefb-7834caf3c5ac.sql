-- Add unique index to prevent duplicate contact_added points awards
CREATE UNIQUE INDEX IF NOT EXISTS user_points_ledger_contact_added_unique 
ON user_points_ledger (user_id, activity_type, (metadata->>'contact_id'))
WHERE activity_type = 'contact_added' AND metadata->>'contact_id' IS NOT NULL;

-- Create function to backfill missing contact_added points
...
SELECT public.backfill_contact_added_points();