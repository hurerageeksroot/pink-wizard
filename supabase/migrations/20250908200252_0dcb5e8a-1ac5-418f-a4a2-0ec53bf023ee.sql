-- Update the useCRMData hook to call milestone bonuses manually after successful contact creation
-- Since we removed the trigger, we need to manually call milestone bonus checks

-- Update gamification functions to call milestone check after points are awarded
...
BEGIN
  ...
  -- Check milestone bonuses after awarding points (only for eligible activities)
  IF p_activity_type NOT LIKE 'milestone_bonus_%' AND 
     p_activity_type NOT LIKE 'streak_bonus_%' AND
     p_activity_type NOT LIKE 'weekly_%bonus' THEN
    BEGIN
      PERFORM public.check_user_milestone_bonuses(p_user_id);
    EXCEPTION WHEN OTHERS THEN
      -- Silently ignore milestone bonus errors to prevent blocking main operations
      NULL;
    END;
  END IF;
