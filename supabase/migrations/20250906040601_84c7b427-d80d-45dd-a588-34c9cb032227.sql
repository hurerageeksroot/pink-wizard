-- Add email sequences table if not exists
CREATE TABLE IF NOT EXISTS email_sequences (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  description TEXT,
  trigger_event TEXT NOT NULL, -- 'user_signup', 'challenge_start', etc.
...
CREATE POLICY "Anyone can view active email sequences" 
ON email_sequences 
FOR SELECT 
...
BEGIN
  ...
  LOOP
    ...
    SELECT content INTO send_result
    FROM http((
      'POST',
      current_setting('app.base_url') || '/functions/v1/send-email',
      ARRAY[
      ...
      jsonb_build_object(
        ...
        'recipientEmail', participant_record.email,
        'recipientUserId', participant_record.id,
        'subjectOverride', p_subject_override,
        'variables', email_variables,
        'idempotencyKey', 'broadcast_' || p_template_key || '_' || participant_record.id || '_' || extract(epoch from now())
      )::text
    ));
  ...
  RETURN jsonb_build_object(
    ...
    'participant_count', participant_count,
    'template_key', p_template_key,
    'message', format('Email broadcast sent to %s challenge participants', participant_count)
  );
END;
$$;

-- Schedule the follow-up reminders cron job (every hour)
SELECT cron.schedule(
  'follow-up-reminders',
  '0 * * * *', -- Every hour at minute 0
  $$
  SELECT net.http_post(
    url := 'https://idwkrddbdyakmpshsvtd.supabase.co/functions/v1/scheduled-email-reminders',
...
-- Schedule the email sequence processor (every 15 minutes)
SELECT cron.schedule(
...
-- Add variables column to email_sequence_logs if not exists
