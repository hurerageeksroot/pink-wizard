-- Fix the ambiguous current_day reference in update_daily_challenge_progress function
...
BEGIN
  ...
  -- Calculate current day (renamed variable to avoid ambiguity)
  calculated_current_day := CASE 
  ...
  -- Update challenge config current_day if needed (now unambiguous)
  UPDATE public.challenge_config 
  SET current_day = GREATEST(calculated_current_day, challenge_record.current_day)
      ...
      overall_progress = LEAST(100.0, 
        (SELECT COUNT(DISTINCT challenge_day)::numeric
         ...
         WHERE udt.user_id = participant_record.user_id
           ...
           AND dtd.is_active = true
           AND udt.challenge_day <= calculated_current_day
        ) / GREATEST(1, calculated_current_day::numeric) * 100
      ),
      current_streak = (
  ...
  RAISE NOTICE 'Daily challenge progress updated successfully for challenge day %', calculated_current_day;
END;
$function$;