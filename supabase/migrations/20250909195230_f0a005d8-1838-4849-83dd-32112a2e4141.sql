-- Update the daily challenge progress function to calculate based on complete days (fixed)
      ...
      IF EXISTS (
        ...
        FROM (
          ...
          WHERE udt.user_id = user_record.user_id 
            AND udt.completed = true
            AND dtd.is_active = true
            AND udt.challenge_day = challenge_day_cursor
          GROUP BY udt.challenge_day
          HAVING COUNT(*) = (
            SELECT COUNT(*) 
            FROM public.daily_tasks_definition 
