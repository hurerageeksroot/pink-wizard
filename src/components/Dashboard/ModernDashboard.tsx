...
export function ModernDashboard({
  ...
  const conversionRate = stats.totalContacts > 0 ? (contacts.filter(c => c.status === 'won').length / stats.totalContacts * 100) : 0;
  ...
  return (
    <div className="space-y-8">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {showDemoData ? 'Viewing sample data to explore features' : 'Add your first contact to get started'}
                </p>
              </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          ...
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div className="min-w-0 flex-1">
                <div className="text-primary-foreground/80 text-xs font-medium mb-1">Total Network</div>
                <div className="text-lg sm:text-xl lg:text-3xl font-bold text-primary-foreground">{stats.totalContacts}</div>
                <div className="text-primary-foreground/70 text-xs mt-1 truncate">contacts managed</div>
              </div>
        ...
        >
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div className="min-w-0 flex-1">
                <div className="text-secondary-foreground/80 text-xs font-medium mb-1">Bookings</div>
                <div className="text-lg sm:text-xl lg:text-3xl font-bold text-secondary-foreground">{stats.bookingsScheduled}</div>
                <div className="text-secondary-foreground/70 text-xs mt-1 truncate">this period</div>
              </div>
        ...
        <Card className="relative overflow-hidden bg-accent border-2 border-accent shadow-elevated">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div className="min-w-0 flex-1">
                <div className="text-accent-foreground/80 text-xs font-medium mb-1">Response Rate</div>
                <div className="text-lg sm:text-xl lg:text-3xl font-bold text-accent-foreground">{stats.responseRate.toFixed(1)}%</div>
                <div className="text-accent-foreground/70 text-xs mt-1 truncate">engagement level</div>
              </div>
        ...
        <Card className="relative overflow-hidden bg-muted/30 border-2 border-primary shadow-elevated">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div className="min-w-0 flex-1">
                <div className="text-muted-foreground text-xs font-medium mb-1">Conversion</div>
                <div className="text-lg sm:text-xl lg:text-3xl font-bold text-foreground">{conversionRate.toFixed(1)}%</div>
                <div className="text-muted-foreground text-xs mt-1 truncate">lead to client</div>
              </div>
      ...
      {/* Revenue Card */}
      <div className="mb-6">
        <Card className="relative overflow-hidden bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200 shadow-elevated">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div className="min-w-0 flex-1">
                <div className="text-amber-800 text-xs font-medium mb-1">Total Revenue</div>
                <div className="text-lg sm:text-xl lg:text-3xl font-bold text-amber-900">
                  ${contacts.reduce((total, contact) => total + (contact.revenueAmount || 0), 0).toLocaleString()}
                </div>
                <div className="text-amber-700 text-xs mt-1 truncate">from won deals</div>
              </div>
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-3">
            ...
            >
              ...
              {(isChallengeParticipant === true) ? 'View Challenge Tasks' : 'View Tasks'}
            </Button>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center p-4 bg-primary/5 rounded-xl border border-primary/10">
                <div className="text-2xl font-bold text-primary mb-1">{todaysActivities.length}</div>
                <div className="text-sm text-muted-foreground">Activities Today</div>
              </div>
              
              <div 
                className="text-center p-4 bg-accent/5 rounded-xl border border-accent/10 cursor-pointer hover:bg-accent/10 transition-colors"
                onClick={() => {
                  navigate('/?tab=contacts&filter=ready');
                  setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 100);
                }}
              >
                <div className="text-2xl font-bold text-accent mb-1">{readyContacts.length}</div>
              ...
              <div className="text-center p-4 bg-secondary/5 rounded-xl border border-secondary/10">
                <div className="text-2xl font-bold text-secondary mb-1">{stats.responseRate.toFixed(0)}%</div>
            ...
            {readyContacts.length > 0 && (
              <div className="mt-6 p-4 bg-gradient-to-r from-primary/10 to-accent/10 rounded-xl border border-primary/20">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-semibold text-primary">Ready for magic! ✨</h4>
                    <p className="text-sm text-muted-foreground">
                      {readyContacts.length} contacts are perfect for your next outreach spell
                    </p>
                  </div>
                  <Button 
                    variant="outline" 
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <Progress value={(stats.coldLeads / stats.totalContacts) * 100} className="w-20 h-2" />
                  <span className="text-sm font-bold min-w-[2rem]">{stats.coldLeads}</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <Progress value={(stats.warmLeads / stats.totalContacts) * 100} className="w-20 h-2" />
                  <span className="text-sm font-bold min-w-[2rem]">{stats.warmLeads}</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <Progress value={(stats.hotLeads / stats.totalContacts) * 100} className="w-20 h-2" />
                  <span className="text-sm font-bold min-w-[2rem]">{stats.hotLeads}</span>
                </div>
              </div>
            </div>
          </CardContent>
            ...
            <CardContent>
              {recentActivities.length > 0 ? (
                      ...
                      <div key={activity.id} className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground truncate">
                            {contact?.name} • {activity.completedAt ? new Date(activity.completedAt).toLocaleDateString() : 'Scheduled'}
                          </div>
                        </div>
                ...
                <div className="text-center py-4 text-muted-foreground">
                  ...
                  <p className="text-sm">Start reaching out to see your activity here</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center p-6 bg-gradient-to-br from-cold/10 to-cold/5 rounded-xl border border-cold/20">
                <div className="text-3xl font-bold text-cold mb-2">{externalCounts.coldOutreach}</div>
                <div className="text-sm text-muted-foreground">Cold Outreach</div>
              </div>
              
              <div className="text-center p-6 bg-gradient-to-br from-warm/10 to-warm/5 rounded-xl border border-warm/20">
                <div className="text-3xl font-bold text-warm mb-2">{externalCounts.nurturingOutreach}</div>
              ...
              <div className="text-center p-6 bg-gradient-to-br from-hot/10 to-hot/5 rounded-xl border border-hot/20">
                <div className="text-3xl font-bold text-hot mb-2">{externalCounts.totalOutreach}</div>
                <div className="text-sm text-muted-foreground">Total Today</div>
              </div>
            </div>
            
            <div className="mt-4 text-center text-xs text-muted-foreground">
              Last updated: {new Date(externalCounts.lastUpdated).toLocaleTimeString()}
            </div>
          </CardContent>
        </Card>
      )}
