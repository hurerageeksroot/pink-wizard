...
export const ChallengeAccessGate: React.FC<ChallengeAccessGateProps> = ({ 
  ...
  const { isActive, startDate, isChallengeParticipant, loading } = useChallenge();
    ...
    return fallback || (
      <Card className="border-muted">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            This feature is exclusive to 75 Hard Mobile Bar Challenge participants.
          </CardDescription>
        </CardHeader>
        <CardContent className="text-center">
          <p className="text-sm text-muted-foreground">
            Join the challenge to unlock this feature and compete with other mobile bar professionals!
          </p>
        </CardContent>
      </Card>
    );
  ...
  const startDateFormatted = startDate ? new Date(startDate).toLocaleDateString('en-US', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  ...
  const featureDescriptions = {
    challenge_tasks: 'Your daily tasks and challenges will be available once the 75-day journey officially begins.',
    community: 'Connect and compete with other challenge participants when the challenge goes live.',
    leaderboards: 'Real-time rankings for points, revenue, and networking will activate on challenge start.',
    rewards: 'Badge earning and prize competitions begin when the challenge officially starts.'
  };
    ...
    <Card className="border-primary/20 bg-primary/5">
      ...
      <CardContent className="space-y-4">
        <Alert className="border-primary/30 bg-primary/10">
          ...
          <AlertDescription>
            <strong>75 Hard Mobile Bar Challenge (Holiday Edition)</strong> starts {startDateFormatted}!
          </AlertDescription>
        </Alert>
