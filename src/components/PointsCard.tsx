...
export function PointsCard() {
  ...
  const { isActive, isChallengeParticipant } = useChallenge();
    ...
    <>
      <Card className="relative overflow-hidden">
        ...
        <CardHeader className="relative pb-3">
          <CardTitle className="flex items-center justify-between text-lg">
            <div className="flex items-center gap-2">
              Points Summary
              {isActive && pointsSummary?.is_challenge_points && (
        ...
        <CardContent className="relative space-y-3 pt-0">
          ...
          {/* Performance Bonuses Info - Only show during active challenge */}
          {isActive && isChallengeParticipant && (
            <div className="mt-4 p-3 bg-gradient-to-r from-primary/10 to-accent/10 rounded-lg border border-primary/20">
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                ...
                <p>
                  • Weekly wins bonus: 150 pts (5+ wins: responses, meetings,
                  deals)
                </p>
      ...
      <PointsDetailsDrawer open={showDetails} onOpenChange={setShowDetails} />
    </>
  );
}
